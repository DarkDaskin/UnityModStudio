...
public static class GameInformationResolver
{
        ...
        out string? errorCode)
    {
        ...
        try
        {
            ...
            var (gameName, company, appInfoFile) = GetAppInfo(gameDataDirectory);
            ...
            gameInformation = new GameInformation
            {
                ...
                PrimaryGameAssemblyFile = GetPrimaryGameAssemblyFile(gameAssemblyFiles),
            };
            return true;
        }
    ...
    private static FileInfo? GetPrimaryGameAssemblyFile(IReadOnlyCollection<FileInfo> assemblyFiles) =>
        assemblyFiles.FirstOrDefault(file => file.Name.Equals("Assembly-CSharp.dll", StringComparison.OrdinalIgnoreCase)) ??
        assemblyFiles.FirstOrDefault(file => file.Name.Equals("Assembly-CSharp-firstpass.dll", StringComparison.OrdinalIgnoreCase));
    ...
    private static (string? gameName, string? company, FileInfo? appInfoFile) GetAppInfo(DirectoryInfo gameDataDirectory)
    {
        ...
        return (appInfo.ElementAtOrDefault(1), appInfo.ElementAtOrDefault(0), appInfoFile);
    }
